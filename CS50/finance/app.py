### pk_429a0061e285466b9f8d3ed5305ca4c7
...
def index():
    ...
    return render_template("index.html", stocks=stocks, total_value=total_value, grand_total=grand_total, )
...
def buy():
    ...
    if request.method == "POST":
        ...
        quote = lookup(symbol)
        if quote is None:
            return apology("Please privide a valid symbol")
        ...
        flash(f"{shares} shares of {symbol} bought for $ {total}")
        ...
        for stock in stocks:
            if stock["symbol"] == symbol:
                if stock["total_shares"] < shares:
                    ...
                    quote = lookup(symbol)
                    if quote is None:
                        return apology("Please eneter a valid symbol")
                    price = quote["price"]
                    total_sale = shares * price

                    db.execute("UPDATE users SET cash = cash + total_sale WHERE id = :user_id", total_sale=total_sale, user_id=session["user_id"])

                    db.execute("INSTERT INTO transactions (user_id, symbol, shares, price) VALUES(:user_id, :symbol, :shares, :price)", user_id=session["user_id"], symbol=symbol, shares=-shares, price=price)
